...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_agent(self, agent_id):
        ...
        ally_adj_dim = self.get_ally_edge_shape()
        enemy_adj_dim = self.get_enemy_edge_shape()
        ally_node_feats_dim = self.get_ally_node_shape()
        enemy_node_feats_dim = self.get_enemy_node_shape()
        #init
        ally_adj = np.zeros(ally_adj_dim, dtype=np.int)
        enemy_adj = np.zeros(enemy_adj_dim, dtype=np.int)
        ally_node_feats = np.zeros(ally_node_feats_dim, dtype=np.float32)
        enemy_node_feats = np.zeros(enemy_node_feats_dim, dtype=np.float32)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    enemy_node_feats[e_id, 0] = 1
                    enemy_node_feats[e_id, 1] = avail_actions[
                        self.n_actions_no_attack + e_id
                    ]  # visible
                    enemy_node_feats[e_id, 2] = dist / sight_range  # distance
                    enemy_node_feats[e_id, 3] = (e_x - x) / sight_range  # relative X
                    enemy_node_feats[e_id, 4] = (e_y - y) / sight_range  # relative Y
                    enemy_node_feats[e_id, 5] = e_x
                    enemy_node_feats[e_id, 6] = e_y
                    enemy_node_feats[e_id, 7] = e_unit.health / e_unit.health_max
            ...
            sigh_ally_index = [agent_id]

            ally_node_feats[agent_id, 0] = 1
            # ally_node_feats[agent_id, 1] = 0
            # ally_node_feats[agent_id, 2] = 0
            # ally_node_feats[agent_id, 3] = 0
            ally_node_feats[agent_id, 4] = x
            ally_node_feats[agent_id, 5] = y
            ally_node_feats[agent_id, 6] = unit.health / unit.health_max
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if (
                    ...
                    sigh_ally_index.append(al_id)
                    ally_feats[i, 0] = 1  # visible
                    ally_feats[i, 1] = dist / sight_range  # distance
                    ally_feats[i, 2] = (al_x - x) / sight_range  # relative X
                    ally_feats[i, 3] = (al_y - y) / sight_range  # relative Y

                    ally_node_feats[al_id, 0] = 1  # visible
                    ally_node_feats[al_id, 1] = dist / sight_range  # distance
                    ally_node_feats[al_id, 2] = (al_x - x) / sight_range  # relative X
                    ally_node_feats[al_id, 3] = (al_y - y) / sight_range  # relative Y
                    ally_node_feats[al_id, 4] = al_x
                    ally_node_feats[al_id, 5] = al_y
                    ally_node_feats[al_id, 5] = al_unit.health / al_unit.health_max
            ...
            #Adj
            if len(sigh_ally_index) > 1:
                ally_adj = self.convert_edge_index_to_adjacency_matrix(
                    np.array(list(permutations(sigh_ally_index, 2)), dtype=int), self.n_agents)

            if len(sigh_enemy_index) > 1:
                enemy_adj = self.convert_edge_index_to_adjacency_matrix(
                    np.array(list(permutations(sigh_ally_index, 2)), dtype=int), self.n_enemies)
        ...
        return agent_obs, ally_node_feats, ally_adj, enemy_node_feats, enemy_adj
    ...
    def get_obs(self):
        ...
        during decentralised execution.
        """
        agents_obs = [self.get_obs_agent(i)[0] for i in range(self.n_agents)]
        return agents_obs

    def get_ally_node_shape(self):
        return self.n_agents, 7
    def get_ally_edge_shape(self):
        return self.n_agents, self.n_agents
    def get_enemy_node_shape(self):
        return self.n_enemies, 8
    def get_enemy_edge_shape(self):
        return self.n_enemies, self.n_enemies

    #agent_obs, ally_node_feats, ally_adj, enemy_node_feats, enemy_adj
    def get_graph_ally_node(self):
        graph_ally_node = [self.get_obs_agent(i)[1] for i in range(self.n_agents)]
        return graph_ally_node

    def get_graph_ally_edge(self):
        graph_ally_edge = [self.get_obs_agent(i)[2] for i in range(self.n_agents)]
        return graph_ally_edge

    def get_graph_enemy_node(self):
        graph_enemy_node = [self.get_obs_agent(i)[3] for i in range(self.n_agents)]
        return graph_enemy_node

    def get_graph_enemy_edge(self):
        graph_enemy_edge = [self.get_obs_agent(i)[4] for i in range(self.n_agents)]
        return graph_enemy_edge

    def convert_edge_index_to_adjacency_matrix(self, edge_index, num_nodes):

        adjacency_matrix = np.zeros((num_nodes, num_nodes))
        for source, target in edge_index:  # Transposed for easy iteration
