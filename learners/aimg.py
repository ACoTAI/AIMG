...
class AIMG_SMAC_Learner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.end_idi_flag = True

        self.rate_survival_loss = self.args.rate_survival_loss
        ...
        if self.args.optimizer == 'adam':
            ...
            self.optimiser = RAdam(params=self.params, lr=args.lr, eps=args.optim_eps, weight_decay=getattr(args, "weight_decay", 0))
        else:
        ...
        self.use_gigm = getattr(args, 'use_gigm', True)
        self.gigm = GIGM(args) if self.use_gigm else None

        self.eval_model_env = None
        self.target_model_env = None
        if not self.use_gigm:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        with th.no_grad():
            ...
            if self.use_gigm:
                _, intrinsic, gigm_aux = self.gigm(batch, chosen_action_qvals.detach(), intrinsic_mask)
                CDI = gigm_aux['extrinsic'].sum(dim=-1, keepdim=True) * intrinsic_mask
                IDI = gigm_aux['intrinsic'].sum(dim=-1, keepdim=True) * intrinsic_mask
                mean_rewards = rewards.sum() / mask.sum()
                rewards_new = rewards + intrinsic
            else:
                ...
                if t_env > self.args.end_idi:
                    self.args.beta1 = self.args.beta1 * 0.99
                    self.args.end_idi += 500000
        ...
        loss = L_td = (1 - self.rate_survival_loss) * loss \
                      + self.rate_survival_loss * (enemy_alive/self.args.n_enemies - mean_alive/self.args.n_agents)
    ...
    def _update_targets(self):
        ...
        if not self.use_gigm and self.target_model_env is not None:
            self.target_model_env.load_state_dict(self.eval_model_env.state_dict())
    ...
    def save_models(self, path):
        ...
        if not self.use_gigm and self.eval_model_env is not None:
